...
|------|--------|------------|
| **00** | `00_generate_spectrograms.py` | **Preparation** | Generate standardized spectrograms from raw SAC waveforms. |
| **01** | `01_a_run_yolo.sh` | Runs YOLOv5 inference on spectrogram images to identify T-phase signals |
...
| **Spectrograms** | `.png` | **Not Included** | Generated by Step 00 from your SAC files |
...
## Usage Guide
Run the pipeline sequentially from **Step 00 (Data Preparation)** and **Step 01 to Step 08**:

### 0: Data Preparation (Preprocessing): Generate Spectrograms
`python src/00_generate_spectrograms.py`

This script converts raw SAC files located in `data/input/waveforms/` into PNG spectrograms. 
- **Consistency**: It is crucial to use this script because it applies the exact signal processing (1.0–20.0 Hz bandpass filter) and visualization scales (-180 to -100 dB) that the YOLOv5 model was trained on.
- **Naming Convention**: It automatically saves images with the required format: `YYYYMMDDhhmm_N.[STATION].png`.
